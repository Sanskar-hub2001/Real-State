...
const Header = () => {
  ...
  return (
    <nav
      className={`${darkMode ? 'dark bg-black' : 'light bg-[#f3f3f3]'
    ...
    >
      ...
      <ul className='lg:flex justify-center items-center gap-8 hidden'>
        {navItems.map(({ link, path }) => (
          <Link
            key={path}
            className='text-black text-[15px] uppercase font-semibold cursor-pointer px-3 py-2 dark:text-white rounded-lg hover:text-white hover:bg-red-600'
            to={path}
            spy={true}
            offset={-100}
            smooth={true}
